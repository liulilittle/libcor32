...
public:
    inline std::string                      Get(const std::string& api, int& status) noexcept { 
        return this->HttpGetOrPostImpl(false, api, NULL, 0, status);
    }
    inline std::string                      Post(const std::string& api, const char* data, size_t size, int& status) noexcept {
